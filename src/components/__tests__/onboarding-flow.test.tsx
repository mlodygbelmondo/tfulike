...
describe("OnboardingFlow", () => {
  ...
  it("stores the username returned by the extension sync", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ ok: true, synced_count: 1 }),
      });
      ...
      likes: [
        {
          ...
          tiktok_url:
            "https://www.tiktok.com/@resolved.from.extension/video/video-1",
    ...
    await user.click(
      screen.getByRole("button", { name: mockDict.onboarding.next }),
    );
    ...
    await user.click(
      screen.getByRole("button", { name: mockDict.onboarding.syncLikes }),
    );
    ...
    await waitFor(() => {
      expect(fetchMock).toHaveBeenNthCalledWith(2, "/api/profile/sync-likes", {
        method: "POST",
  ...
  it("shows the edge function error body instead of the generic wrapper message", async () => {
    ...
    expect(
      await screen.findByText("User profile not found"),
    ).toBeInTheDocument();
  });
